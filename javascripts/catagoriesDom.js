  ...
  elementArray.forEach((element) => {
    myCat.forEach((cat) => {
      if (element.catagoryId === cat.id) {
        document.getElementById(cat.id).innerHTML += `<div class='form-check col-md-3'><input class='form-check-input movie-element' type='checkbox' value='${element.cost}' id='${element.id}' disabled><label class="form-check-label" for="${element.id}">${element.name}</label></div>`;
      };
    });
  });
};
